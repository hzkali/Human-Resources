<?php
/**
 * Consultab\Utility\Styles\Component class
 *
 * @package consultab
 */

namespace Consultab\Utility\Styles;

use Consultab\Utility\Component_Interface;
use Consultab\Utility\Templating_Component_Interface;
use function Consultab\Utility\consultab;
...
class Component implements Component_Interface, Templating_Component_Interface {
	...
	public function action_enqueue_styles() {
		...
		if ( ! empty( $google_fonts_url ) ) {
			wp_enqueue_style( 'consultab-fonts', $google_fonts_url, array(), null ); // phpcs:ignore WordPress.WP.EnqueuedResourceParameters.MissingVersion
		}
		...
		foreach ( $css_files as $handle => $data ) {
			...
			$version = consultab()->get_asset_version( $css_dir . $data['file'] );
	...
	public function filter_resource_hints( array $urls, string $relation_type ) : array {
		if ( 'preconnect' === $relation_type && wp_style_is( 'consultab-fonts', 'queue' ) ) {
			$urls[] = array(
		...
		$handles   = array_filter(
			...
			function( $handle ) use ( $css_files ) {
				...
				if ( ! $is_valid ) {
					...
					_doing_it_wrong( __CLASS__ . '::print_styles()', esc_html( sprintf( __( 'Invalid theme stylesheet handle: %s', 'consultab' ), $handle ) ), 'Consultab 2.0.0' );
				}
				return $is_valid;
			}
		);
	 ...
	 * {@see 'consultab_preloading_styles_enabled'} filter can be used to tweak the return value.
	 *
	...
	protected function preloading_styles_enabled() {
		$preloading_styles_enabled = ! consultab()->is_amp();
		 ...
		 */
		return apply_filters( 'consultab_preloading_styles_enabled', $preloading_styles_enabled );
	}
	...
	protected function get_css_files() : array {
		...
		$css_files = array(
			...
			'consultab-global'     => array(
				'file'   => 'global.min.css',
				'global' => true,
			),
			'consultab-dummy'      => array(
				'file'   => 'dummy.min.css',
				'preload_callback' => '__return_true',
				'global' => true,
			),
			'consultab-button'     => array(
				'file'   => 'button.min.css',
				'global' => true,
			),
			'consultab-comments'   => array(
			...
			'consultab-sidebar'    => array(
				'file'             => 'sidebar.min.css',
				'preload_callback' => function() {
					return consultab()->is_primary_sidebar_active();
				},
				'global' => true,
			),
			'consultab-widgets'    => array(
		 ...
		 */
		$css_files = apply_filters( 'consultab_css_files', $css_files );
	...
	protected function get_google_fonts() : array {
		...
		$google_fonts = array(
			'Open Sans' => array( '300', '300i', '400', '400i','500', '500i', '600', '600i', '700', '700i','800', '800i'),
			'Poppins'     => array( '100', '100i', '200', '200i', '300', '300i', '400', '400i','500', '500i', '600', '600i', '700', '700i','800', '800i','900', '900i'),
		);
		 ...
		 */
		$this->google_fonts = (array) apply_filters( 'consultab_google_fonts', $google_fonts );
