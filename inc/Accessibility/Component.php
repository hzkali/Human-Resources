...
/**
 * Consultab\Utility\Accessibility\Component class
 *
 * @package consultab
 */

namespace Consultab\Utility\Accessibility;
...
class Component implements Component_Interface
{
	...
	public function action_enqueue_navigation_script()
	{
		...
		wp_script_add_data('consultab-navigation', 'async', true);
		wp_script_add_data('consultab-navigation', 'precache', true);
		wp_localize_script(
			'consultab-navigation',
			'consultabScreenReaderText',
			array(
				'expand'   => __('Expand child menu', 'consultab'),
				'collapse' => __('Collapse child menu', 'consultab'),
			)
		);
	}
	...
	public function action_print_skip_link_focus_fix()
	{
		...
		wp_register_script('consultab-minified', '', false, true);
		wp_enqueue_script('consultab-minified', '', array(), false, true);
		wp_add_inline_script('consultab-minified', '/(trident|msie)/i.test(navigator.userAgent)&&document.getElementById&&window.addEventListener&&window.addEventListener("hashchange",function(){var t,e=location.hash.substring(1);/^[A-z0-9_-]+$/.test(e)&&(t=document.getElementById(e))&&(/^(?:a|select|input|button|textarea)$/i.test(t.tagName)||(t.tabIndex=-1),t.focus())},!1);');
	}
