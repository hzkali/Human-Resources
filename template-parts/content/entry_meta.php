...
<div class="consultab-blog-meta">
	<ul class="list-inline">
		<?php
		if (!empty($author_string)) { ?>
			<li class="posted-by">
				<?php
				_e('Posted by : ','consultab');
				/* translators: %s: post author */
				$author_byline = _x('By %s', 'post author', 'consultab');
				if (!empty($time_string)) {
					/* translators: %s: post author */
					$author_byline = _x('%s', 'post author', 'consultab');
				}
				printf(
		...
		if (!empty($parent_string)) { ?>
			<li class="posted-in">
				...
				$parent_note = _x('In %s', 'post parent', 'consultab');
